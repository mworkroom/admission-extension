...
- [ ] Chrome 기본 프로필의 `admission-extension`을 다시 로드하고 버전이 `0.14.0`인지 확인한다.
...
- [ ] 화면 맨 아래에 `문제 기록` 입력창이 표시된다.
- [ ] 활성 작업을 시작하지 않은 상태에서도 문제 내용을 저장할 수 있다.
- [ ] 저장된 문제에 현재 학교·과정·URL·학년도·기록 시각이 자동으로 연결된다.
- [ ] 저장 직후 `최근 문제 기록`에 내용이 표시되고 `기록 N개`가 늘어난다.
- [ ] 문제 내용을 수정하면 최근 목록이 갱신되고 수정 전·후 값이 CSV 이력에 남는다.
- [ ] 삭제 dialog에서 취소하면 기록이 유지되고, 삭제를 확정하면 목록에서만 제거된다.
- [ ] 삭제한 문제의 `user_issue_deleted`와 삭제 전 내용이 CSV에 남는다.
- [ ] Side Panel을 닫았다가 다시 열어도 삭제하지 않은 문제 기록이 유지된다.
- [ ] 문제 기록 저장·수정·삭제가 11개 카드 값과 공통 메모 상태를 바꾸지 않는다.

## 9. 430px Side Panel

- [ ] 상단 기준 패널이 화면에 고정되지 않고 일반 스크롤로 함께 올라간다.
- [ ] 활성 작업 상자, 내보내기 버튼, 실패 요약과 11개 카드에 가로 스크롤이 생기지 않는다.
- [ ] 문제 기록 입력, 최근 기록 카드와 수정·삭제 버튼에 가로 스크롤이 생기지 않는다.

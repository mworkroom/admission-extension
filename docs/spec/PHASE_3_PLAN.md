...
- P3-6 자동·430px 회귀와 실제 Chrome 수동 검증 완료
- 실제 Chrome에서 발견된 새 학년도 재확인 UI 누락을 `0.7.1`에서 보강
- J님이 실제 Chrome `0.7.1`에서 수정 사항 세 항목을 모두 재확인
- Phase 3 완료 버전: `0.7.1`
- 현재 확장 버전: `0.14.0`
- 다음 작업: Phase 4 P4-5·P4-6 실제 Chrome 수동 검증
...
- [x] 실제 Chrome `0.7.1`에서 학년도 전환·복귀와 재확인 UI 재검증
...
로컬 430px harness에서는 확정 Application Fee의 중복 안내가 사라지고, `2027/28` 전환 시 `재확인 필요`와 `이번 학년도 확인 완료` 버튼이 복원되는 것을 확인했다. 이후 J님이 실제 Chrome `0.7.1`에서 학년도 전환 후 재확인 UI, 확정 Application Fee의 중복 안내 제거, 상단 기준 패널의 일반 스크롤을 모두 재확인했다. 모든 완료 기준을 충족해 Phase 3을 완료했다.
